...
public class ProductionRequestController {
    ...
    @GetMapping
    public ResponseEntity<List<ProductionRequest>>
            getAllRequests() {
        return ResponseEntity.ok(
            productionRequestService.getAllRequests());
    }

    // GET /api/production-requests/my/{userId}
    @GetMapping("/my/{userId}")
    public ResponseEntity<List<ProductionRequest>>
            getMyRequests(@PathVariable Long userId) {
        return ResponseEntity.ok(
    ...
    public ResponseEntity<?> createRequest(
        ...
        try {
            ...
            Long requestedById = Long.parseLong(
                request.get("requestedById").toString());
            String notes = request.get("notes") != null
                ? request.get("notes").toString() : "";

            return ResponseEntity.ok(
                productionRequestService.createRequest(
                    itemId, quantity,
                    requestedById, notes));
        } catch (Exception e) {
    ...
    public ResponseEntity<?> rejectRequest(
        ...
        try {
            ...
            String reason = request.get("reason") != null
                ...
                : "No reason provided";
            return ResponseEntity.ok(
                productionRequestService.rejectRequest(
                    id, managerId, reason));
        } catch (Exception e) {
            return ResponseEntity.badRequest()
                .body(Map.of("error", e.getMessage()));
